...
export const employeesSlice = createSlice({
  ...
  reducers: {
    setFilteredByPosted: (state, action) => {
      
        state.filterByPosted = action.payload;
        state.value = applyFilter(state, action);
    },
    setFilteredByArchive: (state, action) => {  
        state.filterByArchive = action.payload;
        state.value = applyFilter(state);
    },
    addNewUser: (state, action) => {  
        state.value = addValue(state, action);
        state.startedValues = state.value;
    },
...
export const { setFilteredByPosted, setFilteredByArchive, addNewUser, editUser } = employeesSlice.actions

export default employeesSlice.reducer

function editValue(state, action) {
  let new_array = [...state.startedValues.filter((item) => item.id != action.payload.id)];
  Object.freeze(action.payload);
  new_array.push(action.payload);
  state.startedValues = [...new_array];
  return applyFilter(state);
}

function addValue(state, action) {
    let new_array = [...state.startedValues];
    Object.freeze(action.payload);
    new_array.push(action.payload);
    state.startedValues = [...new_array];
    return applyFilter(state);
...
function applyFilter(state) {
        let new_array = [...state.startedValues];
            ...
            if(state.filterByPosted == 'all') {
                new_array = [...new_array].sort((a, b) => (a.id > b.id ? -1 : 1))
            } else {
                new_array = [...new_array.filter((item) => item.role == state.filterByPosted)].sort((a, b) => (a.id > b.id ? -1 : 1))
            }
      
            if(state.filterByArchive == false) {
                new_array = [...new_array].sort((a, b) => (a.id > b.id ? -1 : 1))
            } else {
                new_array = [...new_array.filter((item) => item.isArchive == true)].sort((a, b) => (a.id > b.id ? -1 : 1))
            }
